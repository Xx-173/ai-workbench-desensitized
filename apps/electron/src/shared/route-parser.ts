...
export type NavigatorType = 'sessions' | 'sources' | 'skills' | 'automations' | 'projects' | 'pages' | 'settings' | 'workbench'
...
const COMPOUND_ROUTE_PREFIXES = [
  'allSessions', 'flagged', 'archived', 'state', 'label', 'view', 'board', 'sources', 'skills', 'automations', 'projects', 'pages', 'settings', 'workbench'
]
...
export function parseCompoundRoute(route: string): ParsedCompoundRoute | null {
  ...
  if (first === 'workbench' && segments.length === 1) {
...
export function buildCompoundRoute(parsed: ParsedCompoundRoute): string {
  if (parsed.navigator === 'workbench') return 'workbench'
...
function convertCompoundToViewRoute(compound: ParsedCompoundRoute): ParsedRoute {
  if (compound.navigator === 'workbench') return { type: 'view', name: 'workbench', params: {} }
  // Settings
...
function convertCompoundToNavigationState(compound: ParsedCompoundRoute): NavigationState {
  if (compound.navigator === 'workbench') return { navigator: 'workbench', details: null }
  // Settings
  if (compound.navigator === 'settings') {
    if (!compound.details) {
...
function navigationStateToCompoundRoute(state: NavigationState): ParsedCompoundRoute {
  if (state.navigator === 'workbench') return { navigator: 'workbench', details: null }

...
type Bootstrap = { identity: Identity; agents: Agent[]; ownUsage: Usage; departments?: Department[]; users?: User[]; teamUsage?: { total: Usage; byDepartment: Array<Usage & { id: string; name: string }>; byUser: Array<Usage & { id: string; name: string }>; byAgent: Array<Usage & { id: string; name: string }> } }

type Tab = 'run' | 'team' | 'config'
...
export default function AgentWorkbenchPage() {
  ...
  return (
    <div className="h-full overflow-auto bg-background p-5 @container">
      ...
      {tab === 'config' && isAdmin && <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]"><div className="rounded-xl border bg-card p-4"><h2 className="font-medium">Agent Manifest</h2><p className="mt-1 text-xs text-muted-foreground">保存后，工作台和 MCP Server 会读取更新后的配置；真实密钥不要写进 JSON。</p><textarea value={config} onChange={(event) => setConfig(event.target.value)} spellCheck={false} className="mt-3 min-h-[460px] w-full rounded-md border bg-background p-3 font-mono text-xs" /><button onClick={() => void saveConfig()} className="mt-3 rounded-md bg-foreground px-3 py-2 text-sm text-background">校验并保存</button></div><aside className="rounded-xl border bg-card p-4"><h2 className="font-medium">Secret Store</h2><p className="mt-1 text-xs text-muted-foreground">值仅加密存储，不回显给任何成员。</p><input value={newSecret.name} onChange={(event) => setNewSecret({ ...newSecret, name: event.target.value })} className="mt-3 w-full rounded-md border bg-background px-3 py-2 text-sm" placeholder="DIFY_API_KEY" /><input value={newSecret.value} onChange={(event) => setNewSecret({ ...newSecret, value: event.target.value })} type="password" className="mt-2 w-full rounded-md border bg-background px-3 py-2 text-sm" placeholder="粘贴密钥或地址" /><button onClick={() => void saveSecret()} className="mt-2 rounded-md border px-3 py-2 text-sm">加密保存</button></aside></section>}
    </div>
  )
}
